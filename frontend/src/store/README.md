...
1. Create or update a thunk inside `thunks/`, keeping the side-effects close to the relevant API helper. For example, `libraryThunks.ts` now exports both read (`loadLibrary`) and write (`addBookToLibrary`, `addMemo`, `updateMemo`) flows so persistence is centralized, and `publicMemosThunks.ts` wraps both `loadPublicMemos` and the publish/unpublish operations.
